...
class City:
    ...
    def __initialize_city_cross_roads(self):
        for i in range(self.__n):
            for j in range(self.__m):
                ...
                new_vertical_traffic_light = TrafficLight(max_time=30)
                new_horizontal_traffic_light = TrafficLight(max_time=30)
                new_cross_road = CrossRoad(vertical_in_road, vertical_out_road, horizontal_in_road, horizontal_out_road,
                                           new_vertical_traffic_light, new_horizontal_traffic_light)
    ...
    def __initialize_city_cars(self, car_cnt):
        for i in range(car_cnt):
            if random.random() < 0.7:
                input_road_id = random.randint(0, len(self.__input_roads) // 2)
            else:
                input_road_id = random.randint(len(self.__input_roads) // 2 + 1, len(self.__input_roads) - 1)
            if random.random() < 0.7:
                output_road_id = random.randint(0, len(self.__output_roads)//2)
            else:
                output_road_id = random.randint(len(self.__output_roads) // 2 + 1, len(self.__output_roads) - 1)
            input_road = self.__input_roads[input_road_id]
            output_road = self.__output_roads[output_road_id]
            direction = self.__calculate_car_direction(input_road.get_road_start_pos(), output_road.get_road_end_pos())
            new_car = Car(len(self.__cars), input_road.get_road_start_pos(),
                          output_road.get_road_end_pos(), direction, input_road_id)
            new_car.start_new_road(input_road.get_road_length(), input_road.get_road_direction(), input_road_id)
            self.__cars.append(new_car)

    def initialize_city_objects(self, car_cnt):
        self.__clear()
        if len(self.__roads) == 0:
            self.__initialize_city_roads()
        self.__initialize_city_cross_roads()
        self.__initialize_city_cars(car_cnt)
        return self.__roads, self.__cars
    ...
    def forward(self):
        ...
        for car in self.__cars:
            ...
            if car.is_at_the_end_of_the_road() is True:
                ...
                if cur_cross_road.is_road_active(car_next_dir):
                    new_road = cur_cross_road.get_output_road(car_next_dir)
                    car.start_new_road(new_road.get_road_length(), new_road.get_road_direction())
            car.forward()
    ...
    def print_city(self):
        for input_road in self.__input_roads:
            print(input_road)
        print("#############################")
        for output_road in self.__output_roads:
            print(output_road)
        print("#############################")
